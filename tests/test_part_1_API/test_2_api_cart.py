...
@allure.step(
    "Проверка добавления добавления продукта в корзину product_id={product_id} and quantity={quantity}"
)
def test_cart_add(client, product_id, quantity, expected_status):
    ...
    if response.json().get("success"):
        assert (
            response.json()[expected_status]
            == "Success: You have modified your shopping cart!"
        )
    else:
        assert (
            response.json()[expected_status]["warning"]
            == "Warning: Product could not be found!"
        )
...
def test_cart_edit(client, product_id, quantity_add, quantity_edit):
    ...
    assert response.status_code == 200
    assert (
        response.json()["success"] == "Success: You have modified your shopping cart!"
    )
...
def test_cart_remove(client, product_id, quantity_add, use_correct_cart_id, results):
    ...
    assert (
        response_remove.json()["success"]
        == "Success: You have modified your shopping cart!"
    )
    response_edit = client.cart_api.get_item_cart()
    product_exists = any(
        product.get("product_id") == product_id
        for product in response_edit.json().get("products", [])
    )
