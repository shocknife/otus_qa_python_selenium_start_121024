...
def test_post_payment_address(
    client, firstname, lastname, address_1, city, country_id, zone_id
):
    ...
    if not firstname:
        assert (
            response.json()["error"]["firstname"]
            == "First Name must be between 1 and 32 characters!"
        )
    if not lastname:
        assert (
            response.json()["error"]["lastname"]
            == "Last Name must be between 1 and 32 characters!"
        )
    if not address_1:
        assert (
            response.json()["error"]["address_1"]
            == "Address 1 must be between 3 and 128 characters!"
        )
    if not city:
        assert (
            response.json()["error"]["city"]
