...
@pytest.mark.parametrize("product_id, quantity_add, currency, currency_symbol, results",
                         [("40", "1", "GBP", '£', 200),
                          ("40","1", "USD", '$', 200),
                          ("40","1", "EUR", '€', 200),
                          ...
                          ("40","1", " ",None,  400),
                          ("40","1", 44, None, 400)])
def test_post_currency(client, currency, currency_symbol, product_id, quantity_add, results):
    ...
    if currency_symbol is not None:
        ...
        assert all(currency_symbol in text for text in totals_texts), "Не все элементы содержат символ $"
    else:
